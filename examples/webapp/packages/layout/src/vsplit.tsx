...
const DragBar = ({ onChange }: {onChange: onDrag}) => {
  ...
  return (
    <div className="drag-bar-vertical-display">
      <div
        className="drag-bar-vertical"
...
export const VSplit = (props: {children: ReactNode, position?: number[], key: string}) => {
  ...
  const onchange = useCallback(
    debounce((index:number, _: number, offsetY: number) => {
      if (ref.current && offsetY !== 0) {
        const el = ref.current as HTMLDivElement;
        const bbox = el.getBoundingClientRect();
        const loc = 100 * (offsetY - bbox.y) / bbox.height;
        setPos(
          (oldValue) => {
            let lower: number
            let upper: number
  ...
  for (const child of children) {
    innerContent.push(
      <div
        ...
        className="grid-div-column-child-container"
        style={{height: `${pos[i]}%`}}>{child}</div>)
  ...
  return (
    <div className={`grid-div-column`} ref={ref}>
      {innerContent}
    </div>
  );
};
